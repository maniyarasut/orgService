...
public class EmpService {
	...
	public List<Emp> getallEmp() {
		return empRepo.findAllEmp();//Using join fetch to avoid n+1 problem
	}
	...
	public Emp updateEmp(EmpRequest request) throws Exception {
		if (request.getId() != null && request.getId() != 0) {
			...
			if (empOpt.isPresent()) {//Verify the employee
	...
	private Emp empDtoEntityMapping(EmpRequest request, Emp emp) throws Exception {
		...
		if (orgOpt.isPresent()) {//Verify the organization
			Org org = orgOpt.get();
	...
	public ResponseEntity<String> deleteEmp(Long id) {
		Optional<Emp> empOpt = empRepo.findById(id);
		if (empOpt.isPresent()) {//Check whether EmployeeId is valid
			Emp emp = empOpt.get();
			assetRepo.updateEmpAsset(id);
			empRepo.deleteById(id);
			return new ResponseEntity<String>(
					"Employee " + emp.getFirstName() + " " + emp.getLastName() + " deleted successfully ",
					HttpStatus.OK);
		}
